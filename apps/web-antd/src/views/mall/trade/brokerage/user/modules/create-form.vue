...
const [Form, formApi] =
  useVbenForm<MallBrokerageUserApi.BrokerageUserCreateReqVO>({
    commonConfig: {
      componentProps: {
        class: 'w-full',
      },
...
const [Modal, modalApi] = useVbenModal({
  async onConfirm() {
    ...
    // 提交表单
    try {
      await createBrokerageUser(await formApi.getValues());
      // 关闭并提示
  ...
  onOpenChange: async (isOpen: boolean) => {
    ...
    // 重置表单
    await formApi.reset({
      values: { bindUserId: undefined, userId: undefined },
    });
    // 重置用户信息
...
async function handleSearchUser(id: number | string, userType: string) {
  ...
  if (userType === '分销员' && formValues.bindUserId === formValues.userId) {
    ...
    <Form>
      <template #userId="slotProps">
        <InputSearch
          ...
          @search="handleSearchUser($event, '分销员')"
        />
      </template>
