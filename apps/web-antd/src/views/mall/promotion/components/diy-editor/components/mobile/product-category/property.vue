...
defineOptions({ name: 'ProductCategoryProperty' });

const props = defineProps<{ modelValue: ProductCategoryProperty }>();
...
const selectedCategoryIds = computed({
  get: () =>
    buildTreeSelectCategoryValues(formData.value.categoryIds, categories.value),
  set: (value: unknown[]) => {
...
function normalizePageSize() {
  formData.value.pageSize = clampProductCategoryPageSize(
    formData.value.pageSize,
  );
}
            ...
            <template #item="{ element }">
              ...
              >
                ...
                <span class="min-w-0 flex-1 truncate text-sm">
                  {{
                    categories.find((category) => category.id === element)
                      ?.name || `分类 ${element}`
                  }}
                </span>
                <Tooltip title="删除">
      ...
      <Card title="商品样式" class="property-group" :bordered="false">
        ...
        <FormItem label="商品名称" name="fields.name.show">
          <div class="flex gap-2">
            <ColorInput v-model="formData.fields.name.color" /><Checkbox
              v-model:checked="formData.fields.name.show"
            />
          </div>
        </FormItem>
        <FormItem label="商品价格" name="fields.price.show">
          <div class="flex gap-2">
            <ColorInput v-model="formData.fields.price.color" /><Checkbox
              v-model:checked="formData.fields.price.show"
            />
          </div>
        </FormItem>
      </Card>
